...
export class Demo extends Component {
  constructor(props) {
    ...
    this.getFinalAndLatestInterimResult = this.getFinalAndLatestInterimResult.bind(
      this
    );
  ...
  handleFormattedMessage(msg) {
    ...
    if (this.delta_count > 20) {
      this.delta_count = 0;
      const messages = formattedMessages.filter(
        (r) => r.results && r.results.length && r.results[0].final
      );
      const interim = function() {
        const r = formattedMessages[
      ...
      const text = messages.map(m => m.results.map((r, i) => r.alternatives[0].transcript)).reduce((a, b) => a.concat(b), [])[0];
      console.log(text);
      if (text) {
        fetch("/api/v1/translate", { method: 'POST', body: text }).then(async res => {
          const txt = await res.json()
          this.setState({ formattedMessages: formattedMessages.concat(msg), translatedMessages: txt.trans.paragraphs });
        });
      }
    } else {
  ...
  render() {
    ...
    const translatedMessages = this.state.translatedMessages;
    const micBullet =
          ...
          <div className="dropzone _container _container_large">
            <div className="drop-info-container">
              <div className="drop-info">
                ...
                <p>
                  Watson Speech to Text supports .mp3, .mpeg, .wav, .opus, and
            ...
            <ul className="base--ul">
              {micBullet}
              <li className="base--li">
                Upload pre-recorded audio (.mp3, .mpeg, .wav, .flac, or .opus
                only).
              </li>
            ...
            <div className="smalltext">
              *Both US English broadband sample audio files are covered under
            ...
            <div className="flex buttons">
              ...
              >
                <Icon type={audioSource === "upload" ? "stop" : "upload"} />{" "}
                Upload Audio File
              </button>
              ...
              <button
                type="button"
                className={buttonClass}
                onClick={this.handleSample1Click}
              >
                <Icon type={audioSource === "sample-1" ? "stop" : "play"} />{" "}
                Play Sample 1
              </button>
              ...
              <button
                type="button"
                className={buttonClass}
                onClick={this.handleSample2Click}
              >
                <Icon type={audioSource === "sample-2" ? "stop" : "play"} />{" "}
                Play Sample 2
              </button>
            </div>
            ...
            <div className="flex">
              <Translate translatedMessages={translatedMessages} />
            </div>
          </div>
        )}
      </Dropzone>
